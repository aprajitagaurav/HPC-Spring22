    ...
    for (long blk_j = 0; blk_j < n/BLOCK_SIZE; blk_j++){
        for (long blk_p = 0; blk_p < k/BLOCK_SIZE; blk_p++){
            for (long blk_i = 0; blk_i < m/BLOCK_SIZE; blk_i++){
                long startI = blk_i*BLOCK_SIZE;
                long startJ = blk_j*BLOCK_SIZE;
                long startP = blk_p*BLOCK_SIZE;
                for(long j = 0; j < BLOCK_SIZE; j++){
                    for (long p=0; p < BLOCK_SIZE; p++){
                        double B_pj = b[p + startP + (j + startJ) * k];
                        for (long i=0; i < BLOCK_SIZE; i++){
                            double A_ip = a[i + startI + (p + startP) * m];
                            double C_ij = c[i + startI + (j + startJ) * m];
                            C_ij = C_ij + A_ip * B_pj;
                            c[i + startI + (j + startJ) * m] = C_ij;
                        }
                    }
                }
            }
