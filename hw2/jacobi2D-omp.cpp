...
void jacobi(int N, double *u, double *f, int NUM_THREADS) {    
    ...
    for(int it=0 ; it<5000 ; it++) {
        if(result > 1e-6) {
            double sum = 0;
            #ifdef _OPENMP
            #pragma omp parallel for collapse(2) reduction(+:sum) num_threads(NUM_THREADS)
            #endif
            for (int i = 1; i < N + 1; i++) {
                for (int j = 1; j < N + 1; j++) {
                    k++;
                    int idx = (N + 2)*i + j;
                    double residue = (1 / (h*h))*(4 * u[idx] - u[idx - 1] - u[idx + 1] - u[idx - (N + 2)] - u[idx + (N + 2)]) - f[idx];
            ...
            #endif
            result = sqrt(sum);
            cout<<"residue "<<it+1<<" : "<< sqrt(sum)<<endl;
            double *uTemp = uOld;
            uOld = u;
            u = uTemp;
        }
