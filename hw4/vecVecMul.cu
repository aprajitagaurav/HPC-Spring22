...
void vecVecMul_cpu(double* sum_ptr, const double* a, const double* b, long N){
  double sum = 0;
...
__global__ void vecVecMul(double* sum, const double* a, const double* b, long N){
  __shared__ double smem[BLOCK_SIZE];
  int idx = (blockIdx.x) * blockDim.x + threadIdx.x;
...
int main() {
  ...
  vecVecMul<<<Nb,BLOCK_SIZE>>>(sum_d, x_d, y_d, N);
